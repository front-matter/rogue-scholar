...
export function SearchItemCreators({ creators, className, othersLink }) {
  ...
  function getLink(creator) {
    ...
    // Check for ORCID identifier
    let identifiers = _get(creator, "person_or_org.identifiers", []);
    let orcidIdentifier = identifiers.find((id) => id.scheme === "orcid");

    let searchQuery;
    if (orcidIdentifier) {
      // Use ORCID for search if available
      searchQuery = `orcid:${orcidIdentifier.identifier}`;
    } else {
      // Fallback to name search
      searchQuery = `metadata.creators.person_or_org.name:"${creatorName}"`;
    }
